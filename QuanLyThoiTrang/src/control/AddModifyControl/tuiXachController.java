...
public class tuiXachController implements Initializable {
	@FXML
	...
	private String[] loaiTuiXach = { "dài túi", "rộng túi" };
	...
	void clickButton(ActionEvent event) throws ClassNotFoundException, SQLException {
		...
		List<tuiXachModel> tuiXachList = tuiXachService.getListTuiXach();
		...
		if (button.getText().equals("sửa")) {
			...
			tuiXachService.modifyTuiXach(id, myTuiXach, myMauSac, myNhanHieu);
		} else {
			// kiểm tra trùng
			for (tuiXachModel tuixach : tuiXachList) {
				if (tuixach.getId() == Integer.parseInt(myTextField.getText())) {
			...
			tuiXachService.addTuiXach(id, myTuiXach, myMauSac, myNhanHieu);
	...
	public void initialize(URL arg0, ResourceBundle arg1) {
		choiceBox.getItems().addAll(loaiTuiXach);

		choiceBox.valueProperty().addListener(new ChangeListener<String>() {

			@Override
			public void changed(ObservableValue<? extends String> arg0, String arg1, String arg2) {
				myTuiXach = choiceBox.getValue();
			}
